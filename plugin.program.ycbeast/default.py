	...
	if not working == True:
		...
		if len(match) > 0:
			if SEPERATE == 'true':
				...
				addFile('[%s] Jarvis 16 and Above(%s)' % (state, wiz.buildCount('16')), 'showupdate',  '16', themeit=THEME3)
				if SHOW16 == 'true':
					for name, version, url, kodi, icon, fanart in match:
				...
				addFile('[%s] Isengard 15 and Below(%s)' % (state, wiz.buildCount('15')), 'showupdate',  '15', themeit=THEME3)
				if SHOW15 == 'true':
					for name, version, url, kodi, icon, fanart in match:
						kodiv = int(float(kodi))
						if kodiv <= 15:
...
def freshStart(install=None):
	...
	if yes_pressed:
		...
		if install: 
			...
			DIALOG.ok(ADDONTITLE, "The process is complete, you're now back to a fresh Kodi configuration with YCBeast Wizard","Please reboot your system or restart Kodi in order for the changes to be applied.")
			wiz.killxbmc()
